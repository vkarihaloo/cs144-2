...
SELECT COUNT(*) FROM Users WHERE UserID in (SELECT SellerID FROM Items) AND BINARY Location = 'New York';

SELECT COUNT(*) FROM (SELECT ItemID FROM Categories GROUP BY ItemID HAVING COUNT(Category) = 4) A;

SELECT ItemID FROM Items WHERE Currently = (select max(Currently) FROM Items WHERE Ends > '2001-12-20 00:00:01' and Number_of_Bids > 0) and Number_of_Bids > 0;
